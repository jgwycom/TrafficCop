# AGENT_VERSION=2.4-stable
#!/usr/bin/env bash
# trafficcop-manager-merged.sh
# 目标：彻底消灭 node-01 残留、杜绝 HTTP 400、确保 INSTANCE 固定并被面板接收

set -Eeuo pipefail

# ========= Defaults =========
DEFAULT_PG_URL="${PG_URL:-http://127.0.0.1:9091}"   # 可用环境变量预置，也会写入 env
DEFAULT_JOB_NAME="trafficcop"
DEFAULT_PUSH_INTERVAL="10"
DEFAULT_CURL_TIMEOUT="5"
DEFAULT_IFACES="AUTO"
DEFAULT_RUN_DIR="/run/trafficcop"
DEFAULT_METRICS_PATH="${DEFAULT_RUN_DIR}/metrics.prom"
DEFAULT_LOG_LEVEL="info"

# ========= Paths =========
AGENT_DIR="/opt/trafficcop-agent"
AGENT_BIN="${AGENT_DIR}/agent.sh"
ENV_FILE="/etc/trafficcop-agent.env"
SERVICE_FILE="/etc/systemd/system/trafficcop-agent.service"
UNIT_NAME="trafficcop-agent.service"

# ========= Flags =========
FLAG_OVERWRITE="false"
FLAG_KEEP="false"
FLAG_UNINSTALL="false"
FLAG_CLEAR_PG="false"
FLAG_NUKE_PG="false"   # 新增：启动前彻底清空整个 job
FLAG_YES="false"

while [[ $# -gt 0 ]]; do
  case "$1" in
    --overwrite) FLAG_OVERWRITE="true"; shift;;
    --keep) FLAG_KEEP="true"; shift;;
    --uninstall) FLAG_UNINSTALL="true"; shift;;
    --clear-pg) FLAG_CLEAR_PG="true"; shift;;   # 启动后清理兜底
    --nuke-pg) FLAG_NUKE_PG="true"; shift;;     # 启动前清空整个 job（强烈建议）
    -y|--yes) FLAG_YES="true"; shift;;
    -h|--help)
      cat <<'HLP'
Usage: trafficcop-manager-merged.sh [options]

Actions:
  --overwrite     Fresh reinstall (wipe old, then install)
  --keep          Keep current installation (no changes)
  --uninstall     Uninstall everything (service, files)

Optional:
  --nuke-pg       BEFORE starting agent, delete the entire job in Pushgateway
  --clear-pg      AFTER action, clear this instance and/or job in Pushgateway
  -y, --yes       Assume "yes" to prompts (non-interactive)
HLP
      exit 0;;
    *) echo "Unknown option: $1"; exit 2;;
  esac
done

# ========= Helpers =========
log(){ echo "[$(date +'%F %T')] $*"; }
die(){ echo "ERROR: $*" >&2; exit 1; }
need_root(){ [[ ${EUID:-$(id -u)} -eq 0 ]] || die "Please run as root (sudo -i)."; }

stop_disable_service(){ systemctl stop "$UNIT_NAME" 2>/dev/null || true; systemctl disable "$UNIT_NAME" 2>/dev/null || true; }
remove_files(){ rm -f "$SERVICE_FILE" "$ENV_FILE" || true; rm -rf "$AGENT_DIR" || true; systemctl daemon-reload || true; }
ensure_dirs(){ install -d -m 755 "$AGENT_DIR" "$DEFAULT_RUN_DIR"; }

read_env_effective(){
  if [[ -f "$ENV_FILE" ]]; then
    . "$ENV_FILE"
    echo "${PG_URL}|${JOB_NAME}|${INSTANCE}"
  else
    echo "${DEFAULT_PG_URL}|${DEFAULT_JOB_NAME}|UNKNOWN"
  fi
}

# URL 编码（仅编码必要字符，适配 ASCII INSTANCE；中文会被拒，已在输入校验禁止）
raw_urlencode(){
  # 逐字节转义非安全字符
  local s="$1" out="" i c
  for ((i=0;i<${#s};i++)); do
    c="${s:i:1}"
    case "$c" in
      [a-zA-Z0-9._~-]) out+="$c" ;;
      *) printf -v out '%s%%%02X' "$out" "'$c" ;;
    esac
  done
  echo "$out"
}

# —— Pushgateway 清理 —— #
pg_delete_instance(){
  local cfg="$1" PG JOB INST
  PG="$(echo "$cfg" | cut -d'|' -f1)"
  JOB="$(echo "$cfg" | cut -d'|' -f2)"
  INST="$(echo "$cfg" | cut -d'|' -f3)"
  local JENC IENC; JENC="$(raw_urlencode "$JOB")"; IENC="$(raw_urlencode "$INST")"
  curl -fsS -X DELETE "${PG%/}/metrics/job/${JENC}/instance/${IENC}" || true
}
pg_delete_job(){
  local cfg="$1" PG JOB
  PG="$(echo "$cfg" | cut -d'|' -f1)"
  JOB="$(echo "$cfg" | cut -d'|' -f2)"
  local JENC; JENC="$(raw_urlencode "$JOB")"
  curl -fsS -X DELETE "${PG%/}/metrics/job/${JENC}" || true
}

# —— 强制交互输入 INSTANCE（合法性校验） —— #
ask_instance_name(){
  local ans=""
  while true; do
    echo "=============================="
    echo "请输入当前节点的唯一标识 INSTANCE"
    echo "⚠️ 必须全局唯一，只允许字母、数字、点、横杠、下划线"
    echo "示例：node-01, db_02, proxy-kr.03, SzHdy-HK"
    echo "=============================="
    read -r -p "INSTANCE 名称: " ans
    if [[ -z "$ans" ]]; then echo "❌ INSTANCE 不能为空，请重新输入！"; continue; fi
    if [[ ! "$ans" =~ ^[A-Za-z0-9._-]+$ ]]; then echo "❌ INSTANCE 只能包含 [A-Za-z0-9._-]，请重新输入！"; continue; fi
    break
  done
  INSTANCE="$ans"
}

write_env_file(){
  cat >"$ENV_FILE" <<EOF
# Environment for trafficcop-agent
PG_URL="${DEFAULT_PG_URL}"
JOB_NAME="${DEFAULT_JOB_NAME}"
INSTANCE="${INSTANCE}"
PUSH_INTERVAL="${DEFAULT_PUSH_INTERVAL}"
CURL_TIMEOUT="${DEFAULT_CURL_TIMEOUT}"
IFACES="${DEFAULT_IFACES}"
RUN_DIR="${DEFAULT_RUN_DIR}"
METRICS_PATH="${DEFAULT_METRICS_PATH}"
LOG_LEVEL="${DEFAULT_LOG_LEVEL}"
EOF
  chmod 0644 "$ENV_FILE"
}

write_agent_script(){
  cat >"$AGENT_BIN" <<'EOS'
#!/usr/bin/env bash
# AGENT_VERSION=2.4-stable
set -Eeuo pipefail

ENV_FILE="/etc/trafficcop-agent.env"
[[ -f "$ENV_FILE" ]] && . "$ENV_FILE"

PG_URL="${PG_URL:?PG_URL must be set}"
JOB_NAME="${JOB_NAME:-trafficcop}"
INSTANCE="${INSTANCE:?INSTANCE must be set}"
PUSH_INTERVAL="${PUSH_INTERVAL:-10}"
CURL_TIMEOUT="${CURL_TIMEOUT:-5}"
IFACES="${IFACES:-AUTO}"
RUN_DIR="${RUN_DIR:-/run/trafficcop}"
METRICS_PATH="${METRICS_PATH:-${RUN_DIR}/metrics.prom}"
LOG_LEVEL="${LOG_LEVEL:-info}"

log(){ echo "[$(date +'%F %T')] [$1] ${*:2}"; }

ensure_dirs(){ install -d -m 755 "$RUN_DIR"; }
iface_up(){ [[ "$(cat /sys/class/net/$1/operstate 2>/dev/null || echo down)" == "up" ]] && echo 1 || echo 0; }
read_stat(){ cat "/sys/class/net/$1/statistics/${2}_bytes" 2>/dev/null || echo 0; }

list_ifaces(){
  local ret=()
  if [[ "$IFACES" == "AUTO" ]]; then
    local def; def="$(ip route 2>/dev/null | awk '/^default/ {print $5; exit}')"
    [[ -n "$def" ]] && ret+=("$def")
    while IFS= read -r name; do [[ "$name" != "lo" ]] && ret+=("$name"); done < <(ip -o link show up | awk -F': ' '{print $2}')
  elif [[ "$IFACES" == "*" ]]; then
    for n in /sys/class/net/*; do [[ "$(basename "$n")" != "lo" ]] && ret+=("$(basename "$n")"); done
  else read -r -a ret <<<"$IFACES"; fi
  printf '%s\n' "${ret[@]}" | sort -u
}

write_metrics_once(){
  local tmp="${METRICS_PATH}.tmp"
  : > "$tmp"
  LC_ALL=C printf '# HELP traffic_rx_bytes_total Total received bytes.\n# TYPE traffic_rx_bytes_total counter\n' >>"$tmp"
  LC_ALL=C printf '# HELP traffic_tx_bytes_total Total transmitted bytes.\n# TYPE traffic_tx_bytes_total counter\n' >>"$tmp"
  LC_ALL=C printf '# HELP traffic_iface_up Interface state (1=up,0=down).\n# TYPE traffic_iface_up gauge\n' >>"$tmp"

  for ifc in $(list_ifaces); do
    local rx tx up
    rx="$(read_stat "$ifc" rx)"; tx="$(read_stat "$ifc" tx)"; up="$(iface_up "$ifc")"
    # ⚠️ 不要写 instance=；Pushgateway 会根据 URL 自动附加 job/instance
    LC_ALL=C printf 'traffic_rx_bytes_total{iface="%s"} %s\n' "$ifc" "$rx" >>"$tmp"
    LC_ALL=C printf 'traffic_tx_bytes_total{iface="%s"} %s\n' "$ifc" "$tx" >>"$tmp"
    LC_ALL=C printf 'traffic_iface_up{iface="%s"} %s\n' "$ifc" "$up" >>"$tmp"
  done
  # 保证文件末尾有换行
  LC_ALL=C printf '\n' >>"$tmp"
  mv -f "$tmp" "$METRICS_PATH"
}

raw_urlencode(){
  local s="$1" out="" i c
  for ((i=0;i<${#s};i++)); do
    c="${s:i:1}"
    case "$c" in
      [a-zA-Z0-9._~-]) out+="$c" ;;
      *) printf -v out '%s%%%02X' "$out" "'$c" ;;
    esac
  done
  echo "$out"
}

push_metrics(){
  local JENC IENC; JENC="$(raw_urlencode "$JOB_NAME")"; IENC="$(raw_urlencode "$INSTANCE")"
  local url="${PG_URL%/}/metrics/job/${JENC}/instance/${IENC}"
  # 丢弃响应体，避免 curl: (23)；明确 Content-Type
  local code
  code="$(curl -sS -m "${CURL_TIMEOUT}" -o /dev/null -w '%{http_code}' \
    -H 'Content-Type: text/plain; version=0.0.4; charset=utf-8' \
    -X PUT --data-binary @"${METRICS_PATH}" "${url}" || true)"
  if [[ "$code" != "200" && "$code" != "202" ]]; then
    log error "Pushgateway returned HTTP $code -> ${url}"
  fi
}

main_loop(){
  ensure_dirs
  log info "Agent started (JOB=${JOB_NAME}, INSTANCE=${INSTANCE}, PG=${PG_URL}, INTERVAL=${PUSH_INTERVAL}, IFACES=${IFACES})"
  while true; do write_metrics_once; push_metrics; sleep "${PUSH_INTERVAL}"; done
}

main_loop
EOS
  chmod 0755 "$AGENT_BIN"
}

write_service_unit(){
  cat >"$SERVICE_FILE" <<EOF
[Unit]
Description=TrafficCop Pushgateway Agent
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
User=root
EnvironmentFile=-$ENV_FILE
ExecStart=/opt/trafficcop-agent/agent.sh
Restart=always
RestartSec=5

[Install]
WantedBy=multi-user.target
EOF
  chmod 0644 "$SERVICE_FILE"
}

# ========= Workflows =========
install_or_overwrite(){
  log "Installing (fresh) ..."
  ask_instance_name
  stop_disable_service || true
  # —— 启动前：先彻底清空 job，避免旧 node-01 残留 —— #
  if [[ "$FLAG_NUKE_PG" == "true" ]]; then
    local cfg_pre="${DEFAULT_PG_URL}|${DEFAULT_JOB_NAME}|${INSTANCE}"
    pg_delete_instance "$cfg_pre" || true
    pg_delete_job "$cfg_pre" || true
  fi
  remove_files || true
  ensure_dirs
  write_env_file
  write_agent_script
  write_service_unit
  systemctl daemon-reload
  systemctl enable "$UNIT_NAME"
  systemctl start "$UNIT_NAME"
  sleep 2
  systemctl --no-pager --full status "$UNIT_NAME" || true
}

self_check(){
  local cfg; cfg="$(read_env_effective)"
  local PG JOB INST
  PG="$(echo "$cfg" | cut -d'|' -f1)"
  JOB="$(echo "$cfg" | cut -d'|' -f2)"
  INST="$(echo "$cfg" | cut -d'|' -f3)"
  log "Self-check: looking for instance='${INST}' under job='${JOB}' ..."
  # 直接在 PG metrics 里搜你的实例名（PG 会自动加 job/instance）
  if curl -fsS "${PG%/}/metrics" | grep -q "instance=\"${INST}\".*job=\"${JOB}\""; then
    log "✅ Found instance='${INST}' in Pushgateway. Looks good."
  else
    log "❌ Not found yet. Most common causes:"
    log "   1) 另一台机器仍在推旧的 node-01（请那台也升级/停止）。"
    log "   2) 你仓库未更新为本脚本版本（请用本文件覆盖后再试）。"
    log "   3) PG_URL 不对或网络不通。"
    log "   4) 仍然收到 400：请检查 /run/trafficcop/metrics.prom 内容是否只有 iface= 标签。"
    log "—— Preview metrics.prom ——"
    tail -n +1 "${DEFAULT_METRICS_PATH}" 2>/dev/null || true
  fi
}

uninstall_all(){
  log "Uninstalling ..."
  stop_disable_service || true
  remove_files || true
  local cfg="${DEFAULT_PG_URL}|${DEFAULT_JOB_NAME}|UNKNOWN"
  pg_delete_job "$cfg" || true
  log "Uninstalled."
}

# ========= Main =========
need_root
ACTION=""
if [[ "$FLAG_UNINSTALL" == "true" ]]; then ACTION="U"
elif [[ "$FLAG_OVERWRITE" == "true" ]]; then ACTION="O"
elif [[ "$FLAG_KEEP" == "true" ]]; then ACTION="K"; fi
if [[ -z "$ACTION" ]]; then ACTION="O"; fi

case "$ACTION" in
  O) install_or_overwrite; [[ "$FLAG_CLEAR_PG" == "true" ]] && pg_delete_instance "$(read_env_effective)"; self_check;;
  K) log "Keep selected."; [[ "$FLAG_CLEAR_PG" == "true" ]] && pg_delete_instance "$(read_env_effective)"; self_check;;
  U) uninstall_all;;
esac

log "Done."
